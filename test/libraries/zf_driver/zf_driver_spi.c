...
static SPI_Type *spi_index[SPI_NUM] = {SPI1, SPI2, SPI3};
...
uint8 spi_transmit_recieve_byte(spi_index_enum spi_n, uint8 write_buffer)
{
	zf_assert(NULL != write_buffer);
	...
	SPI_CTRL_RX(spi_n, ZF_ENABLE);
  SPI_CTRL_CS(spi_n, ZF_ENABLE);
	while(!SPI_STATE_TX_EMPTY(spi_n));	// 等待发送区空
	SPI_DATA_TX(spi_n, write_buffer);
	while(!SPI_STATE_RX_AVAILABLE(spi_n));	// 等待接收完一个byte
	data = SPI_DATA_RX(spi_n);
	SPI_CTRL_CS(spi_n, ZF_DISABLE);
  SPI_CTRL_RX(spi_n, ZF_DISABLE);
	return data;
}
