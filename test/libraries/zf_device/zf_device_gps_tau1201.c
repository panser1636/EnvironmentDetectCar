...
* 2023-04-07        SeekFree            GPS 修复秒部分错误 修正 RMC 语句解析
********************************************************************************************************************/
/*********************************************************************************************************************
* 接线定义：
*                   ------------------------------------
...
static uint8 gps_gnrmc_parse (char *line, gps_info_struct *gps)
{
    ...
    gps->state = 0;
    if('A' == state)   //A定位有效，V无效                                                         // 如果数据有效 则解析数据
    {
        return_state = 1;
        gps->state = 1;
        gps -> ns               = buf[get_parameter_index(4, buf)]; //维度半球
        gps -> ew               = buf[get_parameter_index(6, buf)]; //经度半球

        latitude                = get_double_number(&buf[get_parameter_index(3, buf)]);//维度ddmm.mmmmm
        longitude               = get_double_number(&buf[get_parameter_index(5, buf)]);//经度ddmm.mmmmm

        gps->latitude_degree    = (int)latitude / 100;                          // dd纬度转换为度分秒
        lati_cent_tmp           = (latitude - gps->latitude_degree * 100); //mm.mmmmm
        gps->latitude_cent      = (int)lati_cent_tmp; //mm.0
        lati_second_tmp         = (lati_cent_tmp - gps->latitude_cent) * 6000;//0.mmmmm*6000
    ...
    gps->time.hour    = (buf[7] - '0') * 10 + (buf[8] - '0');     //UTC时间              // 时间
    gps->time.minute  = (buf[9] - '0') * 10 + (buf[10] - '0');
    gps->time.second  = (buf[11] - '0') * 10 + (buf[12] - '0');
    temp = get_parameter_index(9, buf);                           //UTC日期
...
uint8 gps_data_parse (void)
{
    ...
    do
    {
        if(GPS_STATE_RECEIVED == gps_rmc_state) //gps_uart_callback()函数中接收成功置值
        {
...
void gps_uart_callback (void)
{
    ...
    if(gps_tau1201_state) //gps模块已经初始化
    {
        uint8 dat;
        while(uart_query_byte(GPS_TAU1201_UART, &dat)) //将gps模块的数据写入fifo
        {
        ...
        if('\n' == dat)
        {
            ...
            if(0 == strncmp((char *)&temp_gps[3], "RMC", 3))
            {
                ...
                if(GPS_STATE_PARSING != gps_rmc_state)
                {
                    ...
                    temp_length = fifo_used(&gps_tau1201_receiver_fifo); //查询fifo使用的个数
            ...
            gps_tau1201_flag = 1; //表示rmc的buffer或者gga的buffer已经获得数据
        }
    }
}
...
//    system_delay_ms(50);
		...
		gps_tau1201_state = 1; //gps的初始化状态，1表示gps模块已经初始化
    uart_rx_interrupt(GPS_TAU1201_UART, 1); //串口接收中断，1：打开中断
}
