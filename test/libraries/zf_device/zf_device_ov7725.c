...
// 参数说明     void
// 返回参数     void
// 使用示例     ov7752_vsync_handler();
// 备注信息     通过 zf_device_type.c 的接口调用 用户在使用默认设置时不需要关心
//-------------------------------------------------------------------------------------------------------------------
static void ov7752_vsync_handler (void)
{
...
uint8 ov7725_init (void)
{
    ...
    do
    {
        ...
        set_camera_type(CAMERA_BIN_UART, ov7752_vsync_handler, ov7725_dma_handler, ov7725_uart_handler);
        camera_fifo_init();
        ...
        if(ov7725_get_config(ov7725_get_confing_buffer))
        {
            uart_rx_interrupt(OV7725_COF_UART, 0);
            system_delay_ms(200);
            set_camera_type(CAMERA_BIN_IIC, ov7752_vsync_handler, ov7725_dma_handler, ov7725_uart_handler);
            if(ov7725_iic_init())
            {
